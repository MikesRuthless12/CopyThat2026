...
### WASM plugin runtime (Phase 46 — in progress; 46.1 → 46.6 shipped)
...
- **Still pending: 46.7 wrap.** WASI preview1 imports against granted capabilities (so a plugin with `read_fs:source` can actually call `fd_read` against the source root) defer until then alongside `/review` + `/security-review`, end-to-end smoke, and the post-review final commit that lets full CI run.
