...
- **Cross-tool migration (`copythat migrate` / `export`).** A repository **detector** (restic / Borg / Kopia / CDR-0, from on-disk markers) plus a CDR-0↔CDR-0 migrate path ship in `copythat_chunk::migrate` and the CLI. Full restic / Borg / Kopia importers are gated by hard constraints: every default repo of all three is encrypted, so even enumerating `path → chunks` needs each tool's crypto (AES-CTR/Poly1305, AES-GCM, scrypt/Argon2, keyed-BLAKE2) — plus a MessagePack codec for Borg — none of which are in-tree, and Phase 50's rule is *no new crates*. `migrate <tool>` therefore returns a typed error naming exactly what's required rather than risk a silently-wrong migration. The `cdr-py` PyPI parser and upstream-adoption PRs remain ecosystem follow-ups. See [`docs/spec/CDR-0.md`](docs/spec/CDR-0.md) §11.
