...
/// One file's diff between two snapshots. Mirrors `copythat_chunk::FileDiff`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
/// Diff between two snapshots. Mirrors `copythat_chunk::SnapshotDiff`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
fn change_str(c: copythat_chunk::FileChange) -> &'static str {
    match c {
        copythat_chunk::FileChange::Added => "added",
        copythat_chunk::FileChange::Removed => "removed",
        copythat_chunk::FileChange::Modified => "modified",
        copythat_chunk::FileChange::Unchanged => "unchanged",
    }
}
...
pub async fn repository_diff(
    ...
    let diff = tokio::task::spawn_blocking(move || {
        repo.diff_snapshots(
            copythat_chunk::SnapshotId(from_id),
            copythat_chunk::SnapshotId(to_id),
        )
    })
    .await
...
/// Per-kind snapshot breakdown. Mirrors `copythat_chunk::KindBreakdown`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
/// A growth-curve point. Mirrors `copythat_chunk::GrowthPoint`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
/// A top file. Mirrors `copythat_chunk::TopFile`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
/// The full repository report. Mirrors `copythat_chunk::RepoReport`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
fn report_to_dto(r: copythat_chunk::RepoReport) -> RepoReportDto {
    RepoReportDto {
        stats: RepoStatsDto {
...
pub async fn repository_set_pinned(
    ...
    tokio::task::spawn_blocking(move || {
        repo.set_pinned(copythat_chunk::SnapshotId(snapshot_id), pinned)
    })
    .await
...
pub async fn repository_set_label(
    ...
    let repo = state.repository().ok_or("repository-unavailable")?;
    tokio::task::spawn_blocking(move || {
        repo.set_label(copythat_chunk::SnapshotId(snapshot_id), &label)
    })
    .await
...
pub async fn repository_set_description(
    ...
    let repo = state.repository().ok_or("repository-unavailable")?;
    tokio::task::spawn_blocking(move || {
        repo.set_description(copythat_chunk::SnapshotId(snapshot_id), &description)
    })
    .await
...
pub async fn repository_set_tags(
    ...
    let repo = state.repository().ok_or("repository-unavailable")?;
    tokio::task::spawn_blocking(move || {
        repo.set_tags(copythat_chunk::SnapshotId(snapshot_id), tags)
    })
    .await
...
pub fn repository_compression_get(
    state: tauri::State<'_, AppState>,
) -> copythat_settings::RepoCompressionSettings {
...
pub fn repository_compression_set(
    state: tauri::State<'_, AppState>,
    compression: copythat_settings::RepoCompressionSettings,
) -> Result<(), String> {
...
pub async fn repository_compact(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let mut cb = |p: copythat_chunk::MaintenanceProgress| {
        ...
        match repo.compact(
            copythat_chunk::CompactOptions::default(),
            &cancel_fn,
            &mut cb,
        ) {
            Ok((gc, cr)) => {
...
fn repo_entry_dto(e: &copythat_settings::RepoEntry, active: &str) -> RepoEntryDto {
    RepoEntryDto {
        id: e.id.clone(),
        name: e.name.clone(),
        path: e.path.clone(),
        ...
        active: e.id == active,
        requires_passphrase: copythat_chunk::Repository::requires_passphrase(std::path::Path::new(
            &e.path,
        )),
    }
}
...
fn open_or_reuse(
    ...
    path: &str,
    password: Option<&str>,
) -> Result<std::sync::Arc<copythat_chunk::Repository>, String> {
    ...
    let mut repo = copythat_chunk::Repository::open_existing(std::path::Path::new(path), password)
...
fn register_and_activate(
    ...
    let entry = copythat_settings::RepoEntry {
...
pub fn repository_create(
    ...
    let mut repo =
        copythat_chunk::Repository::create(std::path::Path::new(&path), password.as_deref())
            .map_err(|e| e.to_string())?;
...
/// One source-dashboard row. Mirrors `copythat_chunk::SourceSummary`.
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
...
/// One damaged reference. Mirrors `copythat_chunk::VerifyDamage`.
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
...
/// Outcome of a verify pass. Mirrors `copythat_chunk::VerifyReport`.
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
...
fn verify_report_dto(r: copythat_chunk::VerifyReport) -> VerifyReportDto {
    use copythat_chunk::DamageKind;
    let missing = r
        .damage
        .iter()
...
pub async fn repository_verify(
    ...
    let repo = state.repository().ok_or("repository-unavailable")?;
    let level = if deep {
        copythat_chunk::VerifyLevel::ReadData
    } else {
        copythat_chunk::VerifyLevel::Metadata
    };
    let only = snapshot_id.map(copythat_chunk::SnapshotId);
